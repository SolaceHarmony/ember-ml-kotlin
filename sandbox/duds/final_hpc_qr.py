"""
Final HPC QR decomposition using Modified Gram-Schmidt.

This implementation focuses on numerical stability with proper
thread synchronization in Metal.
"""
...
def final_hpc_qr(A: mx.array) -> tuple[mx.array, mx.array]:
    """
    Final HPC QR decomposition using Modified Gram-Schmidt.
    
    This implementation uses a Metal kernel with proper thread synchronization
    ...
    # Define Metal kernel source string for final HPC QR decomposition
    metal_kernel_source = """
/*****************************************************************************
 *  final_hpc_qr_kernel – QR decomposition with improved numerical stability
 *  
 *  This implementation uses Modified Gram-Schmidt with proper thread
 *  synchronization and numerical stability considerations.
 *  
...
/* ------------------------------------------------------------------------- */
{
    ...
    threadgroup float shared_mem[MAX_THREADS];
    ...
    for (uint j = 0; j < k; ++j)
    {
        ...
        for (uint i = 0; i < j; ++i)
        {
            ...
            shared_mem[thread_position_in_threadgroup.x] = local_dot;
            threadgroup_barrier(mem_flags::mem_threadgroup);
            
            // Perform reduction in shared memory
            for (uint s = threads_per_threadgroup.x / 2; s > 0; s >>= 1) {
                if (thread_position_in_threadgroup.x < s) {
                    shared_mem[thread_position_in_threadgroup.x] += shared_mem[thread_position_in_threadgroup.x + s];
                }
                threadgroup_barrier(mem_flags::mem_threadgroup);
            }
            
            // Thread 0 has the final dot product
            float dot = 0.0f;
        ...
        // Store local sum in shared memory
        shared_mem[thread_position_in_threadgroup.x] = local_norm_sq;
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        // Thread 0 has the final norm squared
        float norm = 0.0f;
        if (thread_position_in_threadgroup.x == 0) {
            float norm_sq = shared_mem[0];
...
def benchmark_final_qr(sizes=[(100, 100), (500, 500), (1000, 1000)], 
                      repeat=3, 
                      compare_with_native=True):
    """
    Benchmark the final QR implementation against different matrix sizes
    ...
    for size in sizes:
        ...
        for i in range(repeat):
            ...
            Q_final, R_final = final_hpc_qr(A)
            end_time = time.time()
            final_times.append(end_time - start_time)
            
        avg_final_time = sum(final_times) / repeat
        print(f"Final QR completed in {avg_final_time:.4f} seconds (avg of {repeat} runs)")
        
        # Check orthogonality and reconstruction
        ortho_error = mx.mean(mx.abs(mx.matmul(Q_final.T, Q_final) - mx.eye(Q_final.shape[0]))).item()
        recon_error = mx.mean(mx.abs(mx.matmul(Q_final, R_final) - A)).item()
        print(f"Orthogonality error: {ortho_error:.6e}")
        print(f"Reconstruction error: {recon_error:.6e}")
        
        result = {
            "size": size,
            "final_time": avg_final_time,
        ...
        if compare_with_native:
            ...
            if not all(np.isnan(native_times)):
                ...
                try:
                    q_diff = mx.mean(mx.abs(Q_final - Q_native)).item()
                    r_diff = mx.mean(mx.abs(R_final - R_native)).item()
                    ...
                    result["speedup"] = avg_native_time / avg_final_time
                except Exception as e:
...
if __name__ == "__main__":
    print("=" * 80)
    print("Final HPC QR Decomposition for MLX")
    print("=" * 80)
    ...
    start_time = time.time()
    Q_small, R_small = final_hpc_qr(A_small)
    end_time = time.time()
    ...
    print("\nRunning benchmarks...")
    results = benchmark_final_qr(sizes=[(50, 50), (100, 100), (200, 200)], repeat=2)
    ...
    for result in results:
        ...
        print(f"{size_str:>10} | {result['final_time']:>10.4f} | {result['ortho_error']:>10.2e} | {result['recon_error']:>10.2e} | {speedup:>10}")
    print("-" * 60)