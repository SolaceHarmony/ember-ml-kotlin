...
#define DEBUG_SIZE 32u      // Size of debug array
    ...
    const uint ltid = ltid_y * TG_SIZE_X + ltid_x;  // Linear threadgroup ID
    
    // Debug values - output our thread positions
    if (tid_x == 0 && tid_y == 0) {
        // Always store magic number first to confirm execution
        debug_out[0] = 0xF00D0001;
    }
    ...
    // Calculate current column index - each x dimension thread handles one column
    const uint col = col0 + tid_x;
    
    // Guard clause: don't process out-of-bounds columns
    if (tid_x >= panel || col >= k) return;
    ...
    // Each y thread handles a subset of rows for the column
    for (uint row = tid_y; row < m; row += threadgroup_position_in_grid.y) {
        // Calculate source index (R part from input)
        uint src_idx = row * scratch_cols + col;
        // Calculate destination index (V part in output)
        uint dst_idx = row * scratch_cols + n + col;
    ...
    // Compute column norm - each thread handles a subset of rows
    float thread_norm_sq = 0.0f;
    for (uint row = tid_y; row < m; row += threadgroup_position_in_grid.y) {
        float val = as_type<float>(scratch[row * scratch_cols + n + col]);
    ...
    // Reduction within threadgroup - first step reduce along y dimension
    if (ltid_y == 0) {
        float sum = norm_sq_partial[ltid_x]; // Start with this thread's value
        for (uint i = 1; i < TG_SIZE_Y; i++) {
            if (ltid_x + i * TG_SIZE_X < TG_SIZE_X * TG_SIZE_Y) {
                sum += norm_sq_partial[ltid_x + i * TG_SIZE_X];
            }
        }
    ...
    // Final reduction and find max norm - done by first thread in threadgroup for this column
    if (ltid_y == 0 && ltid_x == 0) {
        float max_norm_sq = norm_sq_partial[0];
        uint pivot_idx = col;
        
        // Assume we have the maximum norm for this column
    ...
    // Store the pivot and norm info in debug buffer from the first thread
    if (tid_x == 0 && tid_y == 0) {
    ...
    // Only one thread for the pivot column handles this
    // Try simpler Householder reflector computation by letting thread 0 handle it
    if (tid_x == 0 && tid_y == 0) {
...
def qr128_qrp(A: mx.array, want_q: bool = False, debug: bool = True) -> Tuple[Optional[mx.array], mx.array, mx.array]:
    ...
    for col0 in range(0, k, panel):
        ...
        # MODIFIED: Now using 2D grid for panel kernel
        # X dimension for columns, Y dimension for rows
        panel_size = min(panel, k - col0)
        # Launch panel_size x m grid to fully parallelize the matrix
        grid = (panel_size, m, 1)  
        # Use 8x8 threadgroup size for better efficiency
        threadgroup = (8, 8, 1)
        
        if debug:
            print(f"Panel kernel params: grid={grid}, threadgroup={threadgroup}")
            print(f"Panel size: {panel_size}, launching {grid[0]}x{grid[1]} threads")
