...
def test_get_current_backend():
    ...
    assert backend_utils.get_current_backend() == "numpy"
...
def test_set_preferred_backend():
    ...
    try:
        ...
        # We can test setting to 'numpy' again
        preferred = backend_utils.set_preferred_backend("numpy")
        assert backend_utils.get_current_backend() == "numpy"
        assert preferred == "numpy" # Should return the backend that was set
...
def test_print_backend_info(capsys):
    ...
    assert "NumPy" in captured.out # Should mention NumPy as it's the set backend
...
#     assert info['device'] == 'cpu' # NumPy only has cpu