...
def test_features_pca_fit_transform_numpy(numpy_backend): # Use fixture
    """Tests PCA fit_transform with NumPy backend."""
...
def test_features_pca_inverse_transform_numpy(numpy_backend): # Use fixture
    """Tests PCA inverse_transform with NumPy backend."""
    data = _generate_pca_data()
    n_components = 3
    # Use the factory function from features module
    pca_instance = features.pca()
...
def test_features_one_hot_numpy(numpy_backend): # Use fixture
    """Tests features.one_hot with NumPy backend."""
    indices = tensor.convert_to_tensor([0, 2, 1, 0])
    depth = 3
    one_hot_result = features.one_hot(indices, num_classes=depth)
...
def test_features_scatter_numpy(numpy_backend): # Use fixture
