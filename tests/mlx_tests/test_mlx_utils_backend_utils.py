...
def test_get_current_backend():
    ...
    assert backend_utils.get_current_backend() == "mlx"
...
def test_set_preferred_backend():
    ...
    try:
        ...
        # This function tries to set the preferred backend if available
        # We can test setting to 'numpy' and check if it switches (if numpy is installed)
        # Or test setting to 'mlx' again
        set_backend("numpy") # Temporarily switch to ensure set_preferred_backend can switch back
        preferred = backend_utils.set_preferred_backend("mlx")
        assert backend_utils.get_current_backend() == "mlx"
        assert preferred == "mlx" # Should return the backend that was set

        # Test setting to an unavailable backend (should fall back to default or current)
        # This test might be tricky without knowing installed backends.
...
def test_print_backend_info(capsys):
    ...
    assert "MLX" in captured.out # Should mention MLX as it's the set backend
