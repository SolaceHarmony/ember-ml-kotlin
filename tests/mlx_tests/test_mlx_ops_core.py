...
def test_eval():
    ...
    # Create a lazy tensor (backend dependent, but ops.eval should force it)
    x = tensor.random_normal((10, 10))
    # Perform an operation that might be lazy
    y = ops.add(x, 1.0)

    # Before eval, the computation might not have happened yet (backend dependent)
    # Calling eval should force the computation
    ops.eval(y)

    # After eval, the tensor should be computed and usable
    # We can't directly check if it was lazy before, but we can check if it's usable after
