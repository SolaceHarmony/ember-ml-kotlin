...
def test_features_pca_fit_transform_torch(torch_backend): # Use fixture
    """Tests PCA fit_transform with PyTorch backend."""
...
def test_features_pca_inverse_transform_torch(torch_backend): # Use fixture
    """Tests PCA inverse_transform with PyTorch backend."""
    data = _generate_pca_data()
    n_components = 3
    # Use the factory function from features module
    pca_instance = features.pca()
...
def test_features_one_hot_torch(torch_backend): # Use fixture
    """Tests features.one_hot with PyTorch backend."""
    indices = tensor.convert_to_tensor([0, 2, 1, 0])
    num_classes = 3
    one_hot_result = features.one_hot(indices, num_classes=num_classes)
...
def test_features_scatter_torch(torch_backend): # Use fixture
