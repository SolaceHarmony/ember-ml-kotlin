"""
PyTorch + CUDA kernel implementation of SVD using custom CUDA kernels for QR decomposition.
This implementation uses ONLY custom CUDA kernels with no fallbacks to PyTorch operations.
"""
import os
import torch
import numpy as np
...
def load_cuda_extension():
    """Build and load the CUDA extension. Raises an error if CUDA is not available."""
    # Check if CUDA is available
    if not torch.cuda.is_available():
        raise CUDANotAvailableError("CUDA is not available. This implementation requires CUDA.")
    ...
    try:
        ...
        qr_cuda = load(
            name="qr_cuda",
            sources=["qr_cuda_kernel.cu"],
            verbose=True
        )
        print("Successfully loaded CUDA extension.")
    except Exception as e:
        raise RuntimeError(f"Failed to load CUDA extension: {e}")
    finally:
...
qr_cuda = load_cuda_extension()

def svd_with_cuda_kernel(a: torch.Tensor, full_matrices: bool = True, compute_uv: bool = True, 
                        k: int = -1) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
    """
    Compute SVD using custom CUDA kernels for QR decomposition and power iteration.
    This implementation requires CUDA and does not fall back to PyTorch operations.
    ...
    a_tensor = a.to(dtype=torch.float32) if a.dtype != torch.float32 else a
    ...
    if m >= n:
        ...
        q_init = torch.randn((n, k_val), dtype=a_tensor.dtype, device=a_tensor.device)
        q_init, _ = qr_cuda.qr(q_init)
        
        # Perform power iteration to find eigenvectors
        v = qr_cuda.power_iteration(ata, q_init, k_val, 200 if n > 30 else 100, 1e-7)
        ...
        epsilon = torch.tensor(1e-10, device=a_tensor.device)
        ...
        # Compute U = A V S^-1
        epsilon = torch.mul(torch.tensor(1e-10, device=a_tensor.device), torch.max(s))
        ...
        if full_matrices:
            if m > k_val:
                ...
                random_basis = torch.randn((m, remaining_cols), dtype=u.dtype, device=u.device)
                ...
                for i in torch.arange(remaining_cols).tolist():
                    ...
                    scale = torch.where(torch.ge(mask, 0), torch.reciprocal(col_norm), torch.tensor(0.0, device=a_tensor.device))
            ...
            if n > k_val:
                ...
                # Start with random vectors
                random_basis = torch.randn((n, remaining_cols), dtype=v.dtype, device=v.device)
        ...
        # Initialize Q with random orthonormal vectors using QR
        q_init = torch.randn((m, k_val), dtype=a_tensor.dtype, device=a_tensor.device)
        q_init, _ = qr_cuda.qr(q_init)
        
        # Perform power iteration to find eigenvectors
        u = qr_cuda.power_iteration(aat, q_init, k_val, 200 if m > 30 else 100, 1e-7)
...
if __name__ == "__main__":
    print("Testing SVD with PyTorch CUDA Kernel (Pure CUDA Implementation)...")
    ...
    try:
        ...
        U, S, Vh = svd_with_cuda_kernel(A, full_matrices=True, compute_uv=True)
        print("\nResult (SVD with PyTorch CUDA Kernel):")
        print("U:\n", U)
        print("S:\n", S)
        print("Vh:\n", Vh)
        ...
        U_tall, S_tall, Vh_tall = svd_with_cuda_kernel(A_tall)
        ...
        U_wide, S_wide, Vh_wide = svd_with_cuda_kernel(A_wide)
        ...
        U_square, S_square, Vh_square = svd_with_cuda_kernel(A_square)
        ...
        try:
            ...
            print(f"PyTorch's built-in SVD failed: {e}")

    except CUDANotAvailableError as e:
        print(f"\nError: {e}")
        print("This implementation requires CUDA to run.")
    except Exception as e:
        print(f"\nError during SVD with PyTorch CUDA Kernel test: {e}")
        import traceback
        traceback.print_exc()