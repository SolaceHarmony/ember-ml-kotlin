...
class MegaFloatTest {
    ...
    fun testDecimalStringConstructor() {
        ...
        assertEquals("123.45", floatFromString.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
    ...
    fun testArithmeticOperationsUseExponents() {
        ...
        assertEquals("12.0", sum.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
        
        // Test multiplication
        val product = a.mul(b) as MegaFloat
        assertEquals("20.0", product.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
        
        // Test division
        val quotient = a.divide(b) as MegaFloat
        assertEquals("5.0", quotient.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
        ...
        assertEquals("0.75", fracProduct.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
        
        // Test division with fractions
        val fracQuotient = c.divide(d) as MegaFloat
        assertEquals("3.0", fracQuotient.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
    }
    ...
    fun testMixedOperations() {
        ...
        assertEquals("15.5", sum.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
        
        // Test multiplication of float and integer
        val product = float.mul(integer) as MegaFloat
        assertEquals("52.5", product.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
        
        // Test division of float by integer
        val quotient = float.divide(integer) as MegaFloat
        assertEquals("2.1", quotient.toDecimalString().replace(" * 2^(", "e").replace(" * 64)", ""))
    }
}