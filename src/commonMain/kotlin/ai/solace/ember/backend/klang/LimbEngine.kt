...
class LimbEngine constructor(
    ...
    dagEnabled: Boolean = DEFAULT_DAG_ENABLED
) {
    ...
    /** When true, ops are queued (DAG). When false, ops execute eagerly. */
    var dagEnabled: Boolean = dagEnabled

    private sealed interface PendingOp {
        data class Shift(val bits: Int) : PendingOp
        data class And(val mask: IntArray) : PendingOp
        data class Or(val mask: IntArray) : PendingOp
        data class Xor(val mask: IntArray) : PendingOp
    }
    ...
    constructor(value: LimbEngine) : this(
    ...
    fun shift(bits: Int): LimbEngine = apply {
        if (bits == 0) return@apply
        if (dagEnabled) {
            pendingOps.add(PendingOp.Shift(bits))
        } else {
            mantissa = if (bits > 0) shiftLeftBits(mantissa, bits) else shiftRightBits(mantissa, -bits)
            normalize()
        }
    }

    fun shiftLeft(bits: Int): LimbEngine = shift(bits)

    fun shiftRight(bits: Int): LimbEngine = shift(-bits)

    fun flush(): LimbEngine = apply {
        flushPendingOps()
        normalize()
    }

    fun bitAnd(mask: IntArray): LimbEngine = applyMaskOp(PendingOp.And(mask))

    fun bitOr(mask: IntArray): LimbEngine = applyMaskOp(PendingOp.Or(mask))

    fun bitXor(mask: IntArray): LimbEngine = applyMaskOp(PendingOp.Xor(mask))

    private fun applyMaskOp(op: PendingOp): LimbEngine = apply {
        val sanitized = when (op) {
            is PendingOp.Shift -> op
            is PendingOp.And -> PendingOp.And(sanitizeMask(op.mask))
            is PendingOp.Or -> PendingOp.Or(sanitizeMask(op.mask))
            is PendingOp.Xor -> PendingOp.Xor(sanitizeMask(op.mask))
        }
        if (dagEnabled) {
            pendingOps.add(sanitized)
        } else {
            when (sanitized) {
                is PendingOp.And -> applyBitAnd(sanitized.mask)
                is PendingOp.Or -> applyBitOr(sanitized.mask)
                is PendingOp.Xor -> applyBitXor(sanitized.mask)
                is PendingOp.Shift -> { /* handled elsewhere */ }
            }
            normalize()
        }
    }
    ...
    internal fun flushPendingOps() {
        ...
        var netShift = 0
        val optimized = mutableListOf<PendingOp>()
        pendingOps.forEach { op ->
            when (op) {
                is PendingOp.Shift -> netShift += op.bits
                is PendingOp.And -> {
                    val last = optimized.lastOrNull()
                    if (last is PendingOp.And) {
                        optimized[optimized.lastIndex] = PendingOp.And(combineAnd(last.mask, op.mask))
                    } else {
                        optimized.add(PendingOp.And(op.mask.copyOf()))
                    }
                }
                is PendingOp.Or -> {
                    val last = optimized.lastOrNull()
                    if (last is PendingOp.Or) {
                        optimized[optimized.lastIndex] = PendingOp.Or(combineOr(last.mask, op.mask))
                    } else {
                        optimized.add(PendingOp.Or(op.mask.copyOf()))
                    }
                }
                is PendingOp.Xor -> {
                    val lastIndex = optimized.indexOfLast { it is PendingOp.Xor && masksEqual(it.mask, op.mask) }
                    if (lastIndex >= 0) {
                        optimized.removeAt(lastIndex)
                    } else {
                        optimized.add(PendingOp.Xor(op.mask.copyOf()))
                    }
                }
            }
        }
        pendingOps.clear()
        when {
            netShift > 0 -> mantissa = shiftLeftBits(mantissa, netShift)
            netShift < 0 -> mantissa = shiftRightBits(mantissa, -netShift)
        }
        optimized.forEach { op ->
            when (op) {
                is PendingOp.Shift -> error("Shift should have been consumed")
                is PendingOp.And -> applyBitAnd(op.mask)
                is PendingOp.Or -> applyBitOr(op.mask)
                is PendingOp.Xor -> applyBitXor(op.mask)
            }
        }
    }

    private fun sanitizeMask(mask: IntArray): IntArray = IntArray(mask.size) { (mask[it] and MASK) }

    private fun alignMask(mask: IntArray, length: Int): IntArray {
        if (mask.size == length) return sanitizeMask(mask)
        val result = IntArray(length)
        val limit = min(mask.size, length)
        for (i in 0 until limit) {
            result[i] = mask[i] and MASK
        }
        return result
    }

    private fun combineAnd(a: IntArray, b: IntArray): IntArray {
        val len = max(a.size, b.size)
        val aa = alignMask(a, len)
        val bb = alignMask(b, len)
        val out = IntArray(len)
        for (i in 0 until len) {
            out[i] = (aa[i] and bb[i]) and MASK
        }
        return out
    }

    private fun combineOr(a: IntArray, b: IntArray): IntArray {
        val len = max(a.size, b.size)
        val aa = alignMask(a, len)
        val bb = alignMask(b, len)
        val out = IntArray(len)
        for (i in 0 until len) {
            out[i] = (aa[i] or bb[i]) and MASK
        }
        return out
    }

    private fun masksEqual(a: IntArray, b: IntArray): Boolean {
        val len = max(a.size, b.size)
        val aa = alignMask(a, len)
        val bb = alignMask(b, len)
        return aa.contentEquals(bb)
    }

    private fun applyBitAnd(mask: IntArray) {
        val aligned = alignMask(mask, mantissa.size)
        for (i in mantissa.indices) {
            mantissa[i] = mantissa[i] and aligned[i]
        }
    }

    private fun applyBitOr(mask: IntArray) {
        val aligned = alignMask(mask, mantissa.size)
        for (i in mantissa.indices) {
            mantissa[i] = (mantissa[i] or aligned[i]) and MASK
        }
    }

    private fun applyBitXor(mask: IntArray) {
        val aligned = alignMask(mask, mantissa.size)
        for (i in mantissa.indices) {
            mantissa[i] = (mantissa[i] xor aligned[i]) and MASK
        }
    }

    private fun prepareBinaryOperation(other: LimbEngine) {
        flushPendingOps()
        other.flushPendingOps()
    }

    private fun assignFrom(other: LimbEngine) {
    ...
    fun mantissaLimbs(): IntArray {
        flushPendingOps()
        return mantissa.copyOf()
    }

    fun exponentLimbs(): IntArray {
        flushPendingOps()
        return exponent.copyOf()
    }
    ...
    fun add(other: LimbEngine): LimbEngine {
        prepareBinaryOperation(other)
    ...
    fun sub(other: LimbEngine): LimbEngine {
        flushPendingOps()
        val neg = other.copy()
    ...
    fun mul(other: LimbEngine): LimbEngine {
        prepareBinaryOperation(other)
        if (isZero() || other.isZero()) return LimbEngine()
    ...
    fun div(other: LimbEngine): LimbEngine {
        prepareBinaryOperation(other)
        ...
        if (compareAbs(other) == 0 && isFloat == other.isFloat && compareAbsChunks(exponent, other.exponent) == 0 && exponentNegative == other.exponentNegative) {
            val sign = negative.xor(other.negative)
            return LimbEngine(if (sign) "-1" else "1")
        }
        if (isZero()) return LimbEngine()
        val sign = negative.xor(other.negative)
        if (!isFloat && !other.isFloat) {
            val cmp = compareAbsChunks(mantissa, other.mantissa)
            if (cmp < 0) return LimbEngine()
    ...
    fun compareAbs(other: LimbEngine): Int {
        prepareBinaryOperation(other)
    ...
    private fun addFloat(other: LimbEngine): LimbEngine {
        val expA = expAsInt(this)
        val expB = expAsInt(other)
        val mantA: IntArray
        val mantB: IntArray
        ...
        if (negative == other.negative) {
            ...
            if (cmp == 0) {
                return LimbEngine(isFloat = true)
            }
            if (cmp > 0) {
    ...
    private fun expAsInt(target: LimbEngine): Int {
    ...
    companion object {
        ...
        var DEFAULT_DAG_ENABLED: Boolean = true

        inline fun <T> withDagEnabled(enabled: Boolean, block: () -> T): T {
            val prev = DEFAULT_DAG_ENABLED
            DEFAULT_DAG_ENABLED = enabled
            return try { block() } finally { DEFAULT_DAG_ENABLED = prev }
        }

        fun zero(): LimbEngine = LimbEngine()
        fun one(): LimbEngine = LimbEngine(mantissa = intArrayOf(1))

        fun fromDecimalString(value: String): LimbEngine {
            val trimmed = value.trim()
            if (trimmed.isEmpty()) return LimbEngine()
            var negative = false
            var body = trimmed
            if (body.startsWith('-')) {
                negative = true
            ...
            body = body.trim()
            if (body.isEmpty()) return LimbEngine()
        ...
        fun fromBinaryString(value: String): LimbEngine {
            var body = value.trim()
            ...
            return LimbEngine(mantissa = trimChunklist(limbs))
        }
